...
export default function LineupCard({ lineup, index = 0, onClick, fav, onToggleFav, learned, mapName, loggedIn }) {
    ...
    <button className={`ub-card ${learned ? "learned" : ""}`} style={{ animationDelay: `${index * 45}ms`, "--tc": t.color }} onClick={onClick}>
      ...
      <div className="ub-card-top">
        ...
        <div className="ub-card-flags">
          ...
          {loggedIn && learned && <span className="ub-learnflag" title="Learned"><CheckCircle2 size={13} /></span>}
          {loggedIn && <span className={`ub-fav ${fav ? "on" : ""}`} role="button" tabIndex={0} title="Favorite"
